...
import { fetchFileContents, fetchFiles } from "../server";
import parseFrontMatter from 'front-matter';

export async function processFile(file: { path: string, name: string }): Promise<{ title: string; description: string; href: string }> {
    const href = file.path;
    const markdown = await fetchFileContents(href);

    if (markdown) {
        const { attributes } = parseFrontMatter<Attributes>(markdown);
        const { title, description } = attributes;
        return { title, description, href: file.name };
    }

    // Handle the case where markdown is not available or doesn't contain attributes
    return { title: 'Untitled', description: 'No description available', href: file.name };
}

export async function fetchAndProcessFiles(route: string) {
    const files = await fetchFiles(route);
...
export async function getHeader() {
    const { title, meta, svg, items } = await getHomeContent()
...
export async function getHomeContent() {
    ...
    const projects = (await fetchAndProcessFiles('projects')).map(({ title, description, href }) => {
        return { title, description, href: `projects/${href}` };
      });
